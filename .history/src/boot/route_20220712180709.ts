...
import { loginInterception, routesWhiteList } from 'src/config';
...
export default boot(async ({ router }) => {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (hasToken && hasToken != '') {
      ...
      //未登錄狀態
      let flag = 0;
      routesWhiteList.forEach((item: any) => {
        if (item.indexOf(':') != -1) {
          //動態路由
      ...
      if (flag == 1) {
        ...
        console.log('login', toLoginRoute(to.path));
        return next(toLoginRoute(to.path));
      }
    }
  });
});
